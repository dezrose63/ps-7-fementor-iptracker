...
export default function InfoCard({ data }: Props) {
  return (
    <section className="mx-auto grid w-[min(90%,1000px)] grid-cols-1 gap-6 rounded-2xl bg-white p-6 text-center shadow-xl md:grid-cols-4 md:gap-8 md:text-left">
      ...
      <Field label="TIMEZONE" value={data?.location.timezone ?? "—"} showBorder />
...
function Field({ label, value, showBorder }: { label: string; value: string; showBorder?: boolean }) {
  return (
    <div className={`space-y-2 ${showBorder ? 'md:border-r md:border-gray-300 md:pr-8' : ''}`}>
